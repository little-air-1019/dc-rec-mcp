// Ported from Craig (apps/bot/src/modules/recorder/ogg.ts) — read-only reference.
// Hand-writes OggS pages so output matches what cook.sh/oggtracks read.
...
export default class OggEncoder {
  ...
  write(granulePos: number, streamNo: number, packetNo: number, chunk: Buffer, flags = 0) {
    ...
    // And write it out. Stream errors surface via the stream's 'error' event,
    // which the writer's owner handles; we don't swallow them here.
